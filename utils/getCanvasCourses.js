...
exports.getCourses = async () => {
  return getAllCoursesInAccount(1) // first argument is Canvas account ID
    .then(async (courses) => {
      await Promise.all(
        courses.map((course) =>
          getAllModulesInCourse(course.id).then(async (modules) => {
            const populatedModules = await Promise.all(
              modules.map((m) =>
                getAllItemsInCourseModule(course.id, m.id).then(
                  async (items) => {
                    m.items = await Promise.all(
                      items.map((item) => {
                        ...
                        if (item.type === 'Page') {
                          return getCoursePage(course.id, item.page_url).then(
                            (page) => {
                              return {
                                ...item,
                                ...page
            ...
            course.mainModule = populatedModules.find(
              (m) => m.name.toLowerCase() === 'main'
            );
            course.modules = populatedModules.filter(
              (m) => m.name.toLowerCase() !== 'main'
            );
          })
        )
