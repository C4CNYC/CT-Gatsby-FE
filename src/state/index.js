...
import { types as unitTypes } from '../templates/Units/redux/';
// eslint-disable-next-line max-len
import { CURRENT_UNIT_KEY } from '../templates/Units/redux/current-unit-saga';
...
const initialState = {
  ...
  completionCount: 0,
  currentUnitId: store.get(CURRENT_UNIT_KEY),
  showCert: {},
  showCertFetchState: {
    ...defaultFetchState
  },
...
export const updateCurrentUnitId = createAction(
  types.updateCurrentUnitId
);

export const wideScreenStatusChange = createAction(types.wideScreenStatusChange);
export const navigatorPositionChange = createAction(types.navigatorPositionChange);
export const navigatorShapeChange = createAction(types.navigatorShapeChange);
export const scrollToTopStatusChange = createAction(types.scrollToTopStatusChange);
export const fontSizeChange = createAction(types.fontSizeChange);

export const completedUnitsSelector = state =>
  userSelector(state).completedUnits || [];
export const completionCountSelector = state => state[ns].completionCount;
export const currentUnitIdSelector = state => state[ns].currentUnitId;
...
export const shouldRequestDonationSelector = state => {
  ...
  // donations only appear after the user has completed ten units (i.e.
  // not before the 11th unit has mounted)
  if (completedUnits.length < 10) {
    return false;
  }
  // this will mean we have completed 3 or more units this browser session
  // and enough units overall to not be new
...
export const fontSizeIncreaseSelector = state => state[ns].fontSizeIncrease;
...
export const reducer = handleActions(
  {
    ...
    [types.submitComplete]: (state, { payload: { id, challArray } }) => {
      ...
      // to the completedUnits array.
      let submittedunits = [{ id, completedDate: Date.now() }];
      if (challArray) {
        submittedunits = challArray;
      }
      ...
      return {
        ...
        user: {
          ...
          [appUsername]: {
            ...
            completedUnits: uniqBy(
              [
                ...submittedunits,
                ...state.user[appUsername].completedUnits
              ],
              'id'
            )
          }
    ...
    [unitTypes.unitMounted]: (state, { payload }) => ({
      ...state,
      currentUnitId: payload
    }),
      ...
      return {
        ...
        user: {
          ...
          [appUsername]: {
            ...state.user[appUsername],
            completedUnits: uniqBy(
              [...state.user[appUsername].completedUnits, payload],
              'id'
            )
          }
        }
    ...
    [types.wideScreenStatusChange]: (state, { payload: isWideScreen }) => ({
      ...state,
      isWideScreen
    }),
    [types.navigatorPositionChange]: (state, { payload: navigatorPosition }) => ({
      ...state,
      navigatorPosition
    }),
    [types.navigatorShapeChange]: (state, { payload: navigatorShape }) => ({
      ...state,
      navigatorShape
    }),
    [types.scrollToTopStatusChange]: (state, { payload: scrollToTop }) => ({
      ...state,
      scrollToTop
    }),
    [types.fontSizeChange]: (state, { payload: fontSizeIncrease }) => ({
      ...state,
      fontSizeIncrease
    }),
