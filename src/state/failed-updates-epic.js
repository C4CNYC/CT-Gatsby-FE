...
import postUpdate$ from '../templates/Units/utils/postUpdate$';
import { isGoodXHRStatus } from '../templates/Units/utils';
import { backEndProject } from '../../utils/unitTypes';
...
const isSubmitable = failure =>
  failure.payload.unitType !== backEndProject || failure.payload.solution;
...
function failedUpdateEpic(action$, state$) {
  ...
  const flushUpdates = action$.pipe(
    ...
    tap(() => {
      ...
      // delete unsubmitable failed units
