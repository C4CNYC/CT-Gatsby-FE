...
import { updateLegacyCert } from '../../state/settings';
...
const propTypes = {
  completedUnits: PropTypes.arrayOf(
    PropTypes.shape({
      id: PropTypes.string,
...
export class CertificationSettings extends Component {
  ...
  getProjectSolution = (projectId, projectTitle) => {
    const { completedUnits } = this.props;
  ...
  handleSubmitLegacy(formChalObj) {
    ...
    // make an object with keys as unit ids and values as solutions
    let idsToSolutions = {};
    ...
    const completedUnits = this.props.completedUnits;
    let unitsToUpdate = {};
    let newUnitFound = true;
    let oldSubmissions = 0;
    for (let submittedChal of Object.keys(idsToSolutions)) {
      for (let i of completedUnits) {
        if (i.id === submittedChal) {
          if (idsToSolutions[submittedChal] !== i.solution) {
            unitsToUpdate[submittedChal] = idsToSolutions[submittedChal];
          }
          oldSubmissions++;
          newUnitFound = false;
          break;
      ...
      if (newUnitFound && idsToSolutions[submittedChal] !== '') {
        unitsToUpdate[submittedChal] = idsToSolutions[submittedChal];
      }
      newUnitFound = true;
    }
    ...
    return updateLegacyCert({ unitsToUpdate, superBlock });
  }

  renderLegacyCertifications = certName => {
    const { username, createFlashMessage, completedUnits } = this.props;
    const { superBlock } = first(legacyProjectMap[certName]);
    const certLocation = `/certification/${username}/${superBlock}`;
    const unitTitles = legacyProjectMap[certName].map(item => item.title);
    const isCertClaimed = this.getUserIsCertMap()[certName];
    const initialObject = {};
    let filledforms = 0;
    legacyProjectMap[certName].forEach(project => {
      let completedProject = find(completedUnits, function(unit) {
        return unit['id'] === project['id'];
      });
      if (!completedProject) {
    ...
    const options = unitTitles.reduce(
      (options, current) => {
    ...
    const fullForm = filledforms === unitTitles.length;
