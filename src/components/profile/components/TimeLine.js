...
class TimelineInner extends Component {
  ...
  renderCompletion(completed) {
    ...
    const { unitTitle, unitPath, certPath } = idToNameMap.get(id);
    return (
      <tr className='timeline-row' key={id}>
        <td>
          {certPath ? (
            ...
            <Link to={unitPath}>{unitTitle}</Link>
          )}
        </td>
      ...
      <FullWidthRow>
        ...
        {completedMap.length === 0 ? (
          <p className='text-center'>
            No units have been completed yet.&nbsp;
...
function useIdToNameMap() {
  ...
  //   idToNameMap.set(id, {
  //     unitTitle: `${getTitleFromId(id)} Certification`,
  //     certPath: getPathFromID(id)
  //   });
  // }
  edges.forEach(({ node: { id, title, fields: { slug } } }) => {
    idToNameMap.set(id, { unitTitle: title, unitPath: slug });
  });
  return idToNameMap;
}
...
const Timeline = props => {
  ...
  const { sortedTimeline, totalPages } = useMemo(() => {
    const sortedTimeline = reverse(
      sortBy(completedMap, ['completedDate']).filter(unit => {
        return idToNameMap.has(unit.id);
      })
    );
