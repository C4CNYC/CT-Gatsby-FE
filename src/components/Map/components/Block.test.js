...
import mockCompleted from '../../../__mocks__/completedUnitsMock';

test('<Block /> not expanded snapshot', () => {
  const { container } = render(
    <Block
      blockDashedName='block-a'
      units={mockUnitNodes.filter(node => node.block === 'block-a')}
      completedUnits={mockCompleted}
...
test('<Block />  should handle toggle clicks correctly', async () => {
  ...
  expect(container.querySelector('ul').children.length).toBe(
    props.units.length + (props.intro ? 1 : 0)
  );
});
